        ...
        buttons: [{
            ...
            click: function () {
                ...
                if ($('#Mode').val() == 'Insert') {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "Izdavac.aspx/Save",
                        data: "{izdavac:'" + $('#izdavac').val() + "'}",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (msg) {
                            if (msg.hasOwnProperty("d")) {
                                ...
                                if (parseFloat(columns_json[0].col_0) > -1) {
                                    alert("Успјешно сте унијели издавача!");
                                    $('#izdavac').val('');
                    ...
                    $.ajax({
                        type: "POST",
                        url: "Izdavac.aspx/Update",
                        data: "{izdavac:'" + $('#izdavac').val() + "',id:'" + $('#IdItem').val() + "'}",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (msg) {
                            if (msg.hasOwnProperty("d")) {
    ...
    $("#izmjeniItem")
			...
			.click(function () {
			    ...
			    if ($('#gvZanr tbody tr').hasClass('row_selected')) {
			        ...
			        $.ajax({
			            ...
			            success: function (msg) {
			                if (msg.hasOwnProperty("d")) {
			                    ...
			                    $('#izdavac').val(supstanca_json[0].col_1);
			        ...
			        alert('Морате изабрати податке из табеле !');
			    }
			    // $("#dialog-form").dialog("open");
			});
}
