                ...
                "aButtons": [
							            {
							                "sExtends": "xls",
							                "sCharSet": "utf16le",
							                "sFileName": "Spisak Korisnika.xls",
							                "mColumns": [1],
							                "bFooter": false
               							...
               							{
               							    "sExtends": "pdf",
               							    "sFileName": "Spisak Korisnika.pdf",
               							    "sPdfOrientation": "landscape",
               							    "mColumns": [1],
               							    "sPdfMessage": "Spisak Registrovanih Korisnika"
               							}
        ...
        buttons: [{
            ...
            click: function () {
                ...
                if ($('#Mode').val() == 'Insert') {
                        ...
                        success: function (msg) {
                            if (msg.hasOwnProperty("d")) {
                                ...
                                if (parseFloat(columns_json[0].col_0) > -1) {
                                    alert("Uspješno ste unijeli zanr. Možete nastaviti sa unosom slijećeg zanra!");
                                    $('#zanr').val('');
                    ...
                    $.ajax({
                        type: "POST",
                        url: "Zanr.aspx/Update",
                        data: "{zanr:'" + $('#zanr').val() + "',id:'" + $('#IdItem').val() + "'}",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (msg) {
                            if (msg.hasOwnProperty("d")) {
                                ...
                                var columns_json = jQuery.parseJSON(msg.d);
                                if (parseFloat(columns_json[0].col_0) > -1) {
                                    alert('Uspjesno ste promijenili podatke !');
                                    zanr();
                                    $("#dialog-form").dialog("close");
                                }
                            }
    ...
    $("#izmjeniItem")
			...
			.click(function () {
			    ...
			    if ($('#gvZanr tbody tr').hasClass('row_selected')) {
			        ...
			        $.ajax({
			            ...
			            success: function (msg) {
			                if (msg.hasOwnProperty("d")) {
			                    ...
			                    $('#zanr').val(supstanca_json[0].col_1);
			        ...
			        alert('Morate izabrati zanr iz tabele !');
			    }
			    // $("#dialog-form").dialog("open");
			});
}
